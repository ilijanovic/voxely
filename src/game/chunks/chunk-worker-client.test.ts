...
function makeMinimalPayload(chunkX: number, chunkZ: number, requestId: number): ChunkDataPayload & { requestId?: number } {
  return {
    chunkX,
    chunkZ,
...
describe("chunk-worker-client", () => {
  ...
  it("discards stale response when a newer request was made for the same chunk", () => {
    ...
    expect(onPayload).toHaveBeenLastCalledWith(expect.objectContaining({ chunkX: 0, chunkZ: 0, requestId: 2 }));
