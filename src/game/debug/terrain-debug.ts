...
export function logBlockAt(bx: number, by: number, bz: number, label: string): void {
  ...
  if (data) {
    ...
    const posInInstanceList = positionsForType?.some((p) => p.x === bx && p.y === by && p.z === bz) ?? false;
  ...
  if (label.includes("Cell at end of ray") || label.includes("Cell in front")) {
    ...
    if (inVm === false && air) {
      console.log("[block-debug] Interpretation: cell is air in data (no voxel). Either never generated or cleared (e.g. by mining).");
    } else if (inVm === true && noMesh) {
      console.log("[block-debug] Interpretation: block exists in voxelMap but has no mesh – likely refresh/merge bug in chunk-apply or refreshChunkVisibleMeshes.");
    }
  }
}
...
export function logBlockAtCrosshair(
...
export function getSurfaceDecisionReason(
  biome: Biome,
  topY: number,
  surface: BlockType
): string {
